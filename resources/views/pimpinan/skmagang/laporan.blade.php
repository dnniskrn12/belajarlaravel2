...
<head>
    <meta charset="UTF-8" />
    <title>Laporan Data SK Magang</title>
    <style>
        body {
...
<body>
    ...
    <h1 style="text-decoration: underline">LAPORAN SK KERJA SISWA MAGANG</h1>
    <p class="subtitle">Daftar Surat Keterangan Kerja Siswa Magang</p>

    <table>
    <thead>
        <tr>
            <th style="width: 8%;">No</th>
            <th style="width: 27%;">Nomor SK</th>
            <th style="width: 30%;">Nama Siswa</th>
            <th style="width: 20%;">Tanggal SK</th>
            <th style="width: 20%;">Unit Magang</th>
        </tr>
    </thead>
    <tbody>
        ...
        <tr>
            <td>{{ $index + 1 }}</td>
            <td>{{ $sk->no_sk }}</td>
            <td>{{ $sk->magang?->nama_siswa }}</td>
            <td>{{ \Carbon\Carbon::parse($sk->tgl_sk)->format('d M Y') }}</td>
            <td>{{ $sk->unit_magang?->nama_unitmagang }}</td>
        </tr>
        @endforeach
    </tbody>
</table>
